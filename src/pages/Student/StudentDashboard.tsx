...
const StudentDashboard: React.FC = () => {
  ...
  return (
    <>
    <div className="min-h-screen bg-[#f8fafc] dark:bg-gray-950 relative overflow-hidden transition-colors">
      <nav className="bg-[#006837] dark:bg-[#004d28] text-white p-4 shadow-lg sticky top-0 z-20 transition-colors">
      ...
      <main className="max-w-7xl mx-auto p-6 space-y-12">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-gray-100 dark:border-gray-800 pb-10">
          <div>
            <span className="text-[12px] font-bold uppercase tracking-[2px] text-mouau-green dark:text-green-400">Student Portal</span>
            <h1 className="text-4xl font-serif font-bold text-[#1a1a1a] dark:text-white mt-2 tracking-tight">Welcome, {user?.name || 'Student'}</h1>
          </div>
          ...
          <div className="flex gap-4">
            <div className="relative z-10 w-full md:w-auto">
              <button 
                ...
                className="bg-white dark:bg-gray-900 w-full px-6 py-3 rounded-[4px] border border-gray-200 dark:border-gray-800 shadow-sm flex items-center justify-between gap-4 cursor-pointer hover:border-mouau-green dark:hover:border-mouau-green hover:shadow-md transition-all min-w-[200px]"
              >
                <div className="flex items-center gap-3">
                  <Clock className="text-mouau-green dark:text-green-400" size={18} />
                  <span className="text-sm font-bold text-[#666666] dark:text-gray-300 uppercase tracking-wide">{viewLevel || '100L'} Section</span>
                </div>
                <motion.div animate={{ rotate: isLevelOpen ? 180 : 0 }} className="text-gray-400 dark:text-gray-500">
                  <ChevronDown size={16} />
                </motion.div>
              </button>
                ...
                {isLevelOpen && (
                  <>
                    ...
                    <motion.div 
                      ...
                      className="absolute right-0 top-[calc(100%+8px)] w-full min-w-[220px] bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 shadow-2xl rounded-[8px] py-3 z-40 overflow-hidden"
                    >
                      <div className="px-5 pb-3 mb-2 border-b border-gray-50 dark:border-gray-800 flex items-center gap-2">
                        <Layout size={14} className="text-gray-400 dark:text-gray-500" />
                        <span className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest">Select Session Level</span>
                      </div>
                      {['100L', '200L', '300L', '400L'].map((lvl) => (
                        <button
                          key={lvl}
                          onClick={() => { handleLevelChange(lvl); setIsLevelOpen(false); }}
                          className={`w-full text-left px-5 py-3 text-sm font-bold transition-all flex items-center justify-between group ${viewLevel === lvl ? 'text-mouau-green dark:text-green-400 bg-mouau-green/5 dark:bg-green-400/10' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 hover:text-mouau-green dark:hover:text-green-400'}`}
                        >
                          <span className="translate-x-0 group-hover:translate-x-1 transition-transform">{lvl} Courseware</span>
                          {viewLevel === lvl && (
                            <motion.div layoutId="activeLevelIndicator" className="w-2 h-2 rounded-full bg-mouau-green dark:bg-green-400 shadow-[0_0_8px_rgba(0,104,55,0.5)]" />
                          )}
                        </button>
                      ))}
                    </motion.div>
        ...
        <section>
          <div className="flex items-center justify-between mb-8">
            ...
            <div className="flex items-center gap-2 text-mouau-green dark:text-green-400 text-sm font-bold uppercase tracking-widest">
              <Layout size={16} />
          ...
          {loading ? (
             <div className="space-y-4">
               {[1,2,3].map(i => (
                 <div key={i} className="h-20 bg-gray-100 dark:bg-gray-800 animate-pulse rounded-[4px]" />
               ))}
             </div>
          ) : displayMaterials.length === 0 ? (
            <div className="bg-gray-50 dark:bg-gray-900 border border-dashed border-gray-300 dark:border-gray-800 rounded-lg p-20 text-center">
              ...
              <h3 className="text-xl font-serif font-bold text-gray-900 dark:text-white">No courses found</h3>
            ...
            <div className="space-y-4">
              {displayMaterials.map((material) => (
                <div key={material.id} className="bg-white dark:bg-gray-900 rounded-[4px] border border-gray-100 dark:border-gray-800 shadow-sm overflow-hidden flex flex-col sm:flex-row items-center justify-between p-6 hover:border-mouau-green dark:hover:border-mouau-green transition-all gap-4">
                  <div className="flex-1 w-full text-left">
                     <div className="flex items-center gap-4 mb-2">
                       <span className="font-bold text-mouau-green dark:text-green-400 tracking-tight font-mono bg-mouau-green/5 dark:bg-green-400/10 px-3 py-1 rounded">{material.courseCode}</span>
                       ...
                       <span className="text-xs text-gray-400 dark:text-gray-500 font-medium uppercase tracking-wider hidden sm:inline">{material.semester} Semester</span>
                     </div>
                     <h3 className="font-serif font-bold text-xl text-gray-900 dark:text-white">{material.courseTitle}</h3>
                     <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Uploaded by: {material.lecturerName}</p>
                  </div>
                  <button
        ...
        {searchTerm === '' && (
          <section className="bg-gray-50 dark:bg-gray-900 rounded-[4px] p-10 border border-gray-100 dark:border-gray-800">
            <div className="flex items-center justify-between mb-8">
              <h2 className="text-xl font-serif font-bold text-[#1a1a1a] dark:text-white flex items-center gap-3">
            ...
            <div className="space-y-6">
              {bulletins.length === 0 ? (
                <div className="text-center py-10 text-gray-400 dark:text-gray-500 italic font-serif">
                ...
                bulletins.map((bulletin) => (
                  <motion.div 
                    ...
                    key={bulletin.id} 
                    className="flex gap-6 p-6 rounded-[4px] bg-white dark:bg-gray-950 border border-gray-100 dark:border-gray-800 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden group"
                  >
                    <div className="absolute top-0 left-0 w-1 h-full bg-mouau-green group-hover:w-2 transition-all" />
                    <div className="flex-1">
                      <div className="flex items-center justify-between mb-2">
                        <h4 className="font-serif font-bold text-[#1a1a1a] dark:text-gray-100 text-lg">Broadcast from {bulletin.lecturerName}</h4>
                        <span className="text-[10px] text-gray-400 dark:text-gray-500 font-bold uppercase tracking-widest">{bulletin.createdAt?.toDate ? new Date(bulletin.createdAt.toDate()).toLocaleDateString() : 'Just now'}</span>
                      </div>
                      <p className="text-sm text-[#666666] dark:text-gray-400 leading-relaxed font-medium">{bulletin.content}</p>
                    </div>
                  </motion.div>
                ))
              )}
