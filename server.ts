...
5. End your response with the exact signature phrase: "Generated by Demic_AI" on a new line.`
  ...
  app.post('/api/ai-doc-chat', async (req, res) => {
    ...
    const { fileUrl, courseCode, courseTitle, query, history } = req.body;
    ...
    try {
      const fileResponse = await fetch(fileUrl);
      if (!fileResponse.ok) throw new Error('Failed to fetch document from secure storage');
...
INSTRUCTIONS:
1. Answer the student's question accurately using ONLY the information provided in the attached PDF document.
2. If the user asks about a specific detail not in the PDF, politely respond that the current lecture document does not cover that specific detail.
3. Keep your answer direct, helpful, and concise. Format nicely in markdown.
4. End your response with the signature: "Generated by Demic_AI" on a new line.`
            },
            {
              inlineData: {
                mimeType: "application/pdf",
      ...
      return res.status(200).json({ 
        answer: isInvalidKey 
          ...
          : "Sorry, I encountered an error while trying to read the document. Please try again later."
      });
    }
  });
